...
class ChatModel extends ChangeNotifier {
  ...
  Future<String>? generatePostWorkoutMessage(
    Map<String, dynamic> gameState, BuildContext context
  ) async {
    ...
    try {
      ...
      const pollingInterval = Duration(
          seconds:
              2); // Adjust for longer interval between retrieve times. 2 seconds worked better than 0 during testing
      ...
      while (runStatus != 'completed' && retries < maxRetries) {
        ...
        logger.i("RequiredAction: ${updatedRun.requiredAction}");
        if (runStatus == 'requires_action') {
          if (context.mounted) {
            await handleRequiredAction(updatedRun, context);
          }
